...
fun solvePartTwo(): String {
    ...
    return claims.find { !it.overlap }?.id ?: "Fucked it"
}
 ...
 */
fun parseClaims(): List<Claim> {
    return File("src/day3/input.txt").readLines().map { line ->
        claimPattern.find(line)?.let {
            val (id, x, y, w, h) = it.destructured
            Claim(id, x.toInt(), y.toInt(), w.toInt(), h.toInt())
        } ?: throw IllegalArgumentException("Couldn't parse claim $line")
    }
}

data class Claim(val id: String, val startX: Int, val startY: Int, val width: Int, val height: Int, var overlap: Boolean = false)